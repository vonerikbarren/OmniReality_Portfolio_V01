...
export default class OmniMixerPanel {
  ...
  _playChannel (ch) { this._audioEls[ch]?.play().catch(() => {}) }
  _stopChannel (ch) { const a = this._audioEls[ch]; if (a) { a.pause(); a.currentTime = 0 } }
  ...
  async _nextTrack (ch) {
    ...
    await this._loadTrackIntoChannel(ch, nextSlot)
  ...
  async _loadVideoIntoSlot (slot) {
    ...
    this._videoEls[slot].play().catch(() => {})
  }
